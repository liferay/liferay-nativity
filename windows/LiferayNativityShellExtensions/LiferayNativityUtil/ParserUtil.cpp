...
bool ParserUtil::ParseNativityMessageList(wstring* message, vector<NativityMessage*>* messages)
{
...
bool ParserUtil::SerializeMessage(NativityMessage* nativityMessage, std::wstring* result)
{
	if(result == 0)
	{
		return false;
	...
	result->append(QUOTE);
	result->append(nativityMessage->GetCommand()->c_str());
	result->append(QUOTE);
	...
	if(!IsList(nativityMessage->GetValue()))
	{
		result->append(QUOTE);
	}
	
	result->append(nativityMessage->GetValue()->c_str());
